...
import { Button, Card, Heading, getSize } from '~/components/common'
...
export default function Login({ onConfirm, onCancel, onReject }: LoginProps) {
    ...
    return (
        <Card className='grid gap-20' size='sm'>
            ...
            <div className='grid p-10 rounded-[0.5rem] bg-black-secondary'>
                {wallets.map((wallet, i) => (
                    ...
                    >
                        ...
                        <div className={classNames(
                            ...
                            'after:content-[""] after:absolute after:w-[0.5rem] after:h-[0.5rem] after:rounded-[1px] after:bg-primary after:bg-opacity-0 after:transition-background after:duration-100',
                            { 'before:border-primary after:bg-opacity-100': selectedWallet?.id === wallet.id },
                        )} />
                    </button>
                ))}
            </div>
            <Button variant="primary" disabled={!selectedWallet} onClick={login} className={classNames({ 'cursor-not-allowed': isLoggingIn })} gap="lg">
                Connect wallet {isLoggingIn && <UtilSpinner />}
            </Button>
        </Card>
    )
}
